...
| `EVAL_FAST_REJECT` | `0`/`1` | Eval phase (challenge `eval_circuit`): `1` stops at the first failing batch (clean/dirty verdict only). Default `0` so scoring/submission runs are complete. ~1.5× on dirty candidates today (capped by the ~9 s upfront input derivation); a lazy-derivation rewrite would reach ~10×. `island.sh validate` sets it to `1` by default. **Note:** this lives in the challenge repo and is reset by `ecdsafail sync` — re-apply from `patches/eval_fast_reject.diff`. |
